...
const Auth: React.FC = () => {
  ...
  const { user, loginOffline, canLoginOffline } = useAuth();
  ...
  // Check if offline login should be shown
  useEffect(() => {
    ...
    if (isOffline && canLoginOffline) {
      console.log('✅ Showing offline login');
      setShowOfflineLogin(true);
    } else if (!isOffline) {
      console.log('🌐 Online - hiding offline login');
      setShowOfflineLogin(false);
    }
  }, [isOffline, canLoginOffline]);
  ...
  // Offline login handlers
  const handleOfflineLogin = async (user: any, cachedData: any) => {
    try {
      console.log('Offline login successful:', user.email);
      // The user is already set by the loginOffline function
      // Navigate to the appropriate dashboard
      const defaultRoute = getDefaultRouteForRole(user.role);
      console.log('🔄 Offline login - redirecting to:', defaultRoute);
      navigate(defaultRoute, { replace: true });
    } catch (error) {
  ...
  const handleCancelOfflineLogin = () => {
    setShowOfflineLogin(false);
  };
  ...
  return (
    <div className="min-h-screen flex flex-col relative">
      {/* Show offline login if offline and cached auth is available */}
      {showOfflineLogin ? (
        <OfflineLogin
          onOfflineLogin={handleOfflineLogin}
          onCancel={handleCancelOfflineLogin}
        />
      ) : (
        <>
