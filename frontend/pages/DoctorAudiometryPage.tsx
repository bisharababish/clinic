// DoctorAudiometryPage.tsx - With comprehensive skeleton loading
...
const DoctorAudiometrySkeletonLoading = ({ isRTL }: { isRTL: boolean }) => (
...
const DoctorAudiometryPage: React.FC = () => {
    const { user } = useAuth();
    ...
    const [selectedImage, setSelectedImage] = useState<AudiometryImage | null>(null);
    const [filterDate, setFilterDate] = useState<string>('');
    const [imageZoom, setImageZoom] = useState<number>(100);
    const [imageRotation, setImageRotation] = useState<number>(0);
    const [deletingId, setDeletingId] = useState<number | null>(null);
    const [confirmDeleteImage, setConfirmDeleteImage] = useState<AudiometryImage | null>(null);
    ...
    const fetchAudiometryImages = async () => {
        ...
        try {
            setLoading(true);
            setError(null);
            console.log('Fetching Audiometry images...');
            ...
            if (audiometryError) {
                console.error('Error fetching Audiometry images:', audiometryError);
                throw new Error(audiometryError.message);
            }
            ...
            console.error('Error in fetchAudiometryImages:', err);
            setError(err instanceof Error ? err.message : 'Failed to load Audiometry images');
        } finally {
            setLoading(false);
        ...
        const initializeComponent = async () => {
            try {
                ...
                if (user.role !== 'doctor' && user.role !== 'admin') {
                    setError(t('admin.accessDenied') || 'Access denied. Only doctors and administrators can view Audiometry images.');
                    setInitializing(false);
                    setLoading(false);
                    return;
                }

                // Fetch Audiometry images
                await fetchAudiometryImages();
            } catch (err) {
    ...
    const handleViewImage = (image: AudiometryImage): void => {
        setSelectedImage(image);
        setImageZoom(100);
        setImageRotation(0);
    };

    const handleDownloadImage = async (image: AudiometryImage): Promise<void> => {
        try {
            if (!image.imageUrl) {
                toast({
    ...
    const handleDeleteImage = async (image: AudiometryImage): Promise<void> => {
        ...
        try {
            ...
            if (image.image_url) {
                ...
                const match = image.imageUrl.match(/\/audiometry-images\/(.+?)(?:\?|$)/);
                if (match && match[1]) {
            ...
            // Update UI
            setAudiometryImages(prev => prev.filter(x => x.id !== image.id));

            toast({
                title: 'Audiometry Image Deleted',
                description: 'The audiometry image and file were deleted successfully.',
                variant: 'default',
    ...
    if (loading) {
        return <LoadingSkeletonWithText text={t('doctorPages.loadingAudiometryImages') || 'Loading Audiometry images...'} />;
    }
                        ...
                        <div className="flex items-center justify-between">
                            <div>
                                <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
                                    ...
                                    {t('doctorPages.audiometryImages') || 'Audiometry Images'}
                                </h1>
                                <p className="mt-1 text-sm text-gray-600">
                                    {t('doctorPages.audiometryImagesDesc') || 'View and analyze patient Audiometry images'}
                                </p>
                            </div>
                            <button
                                onClick={handleRefresh}
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                ...
                <div className="bg-white rounded-lg shadow-sm border p-6 mb-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="relative">
                            ...
                            <input
                                type="text"
                                placeholder={t('doctorPages.searchPatientsAudiometry') || 'Search patients or notes...'}
                                value={searchTerm}
                                onChange={(e) => {
                            ...
                            <div key={image.id} className="bg-white rounded-lg shadow-sm border overflow-hidden hover:shadow-md transition-shadow">
                                ...
                                <div className="p-4">
                                    ...
                                    <div className="space-y-1 text-sm text-gray-600 mb-4">
                                        ...
                                        <p><span className="font-medium">{t('doctorPages.requestingDoctor') || 'Requesting Doctor'}:</span> {image.requesting_doctor || 'N/A'}</p>
                                    </div>

                                    {image.notes && (
                                        <div className="mb-4">
                                            <p className="text-sm text-gray-600 line-clamp-2">
                                                <span className="font-medium">{t('audiometry.notes.title') || 'Notes'}:</span> {image.notes}
                                            </p>
                                        </div>
                                    )}
                                            ...
                                            <AlertDialog>
                                                ...
                                                <AlertDialogContent className={`${isRTL ? 'text-right' : 'text-left'} ${isRTL ? '[&>button]:left-4 [&>button]:right-auto' : ''}`} dir={isRTL ? 'rtl' : 'ltr'}>
                                                    <AlertDialogHeader className={isRTL ? 'text-right' : 'text-left'}>
                                                        <AlertDialogTitle className={isRTL ? 'text-right' : 'text-left'}>{t('doctorPages.deleteConfirmTitleAudiometry') || 'Delete Audiometry Image?'}</AlertDialogTitle>
                                                        <AlertDialogDescription className={isRTL ? 'text-right' : 'text-left'}>
                                                            {t('doctorPages.deleteConfirmAudiometry') || 'Are you sure you want to delete this Audiometry image? This action cannot be undone.'}
                                                        </AlertDialogDescription>
                                                    </AlertDialogHeader>
                ...
                {filteredImages.length === 0 && !loading && (
                    <div className="text-center py-12">
                        ...
                        <p className="text-gray-500 text-lg">{t('doctorPages.noAudiometryImagesFound') || 'No Audiometry images found matching your criteria.'}</p>
                        <p className="text-gray-400 text-sm mt-2">
                            {audiometryImages.length === 0
                                ? t('doctorPages.noAudiometryImagesUploaded') || 'No Audiometry images have been uploaded yet.'
                                ...
                                <div className="p-3 sm:p-4 space-y-3 sm:space-y-4">
                                    ...
                                    <div>
                                        ...
                                        <div className="space-y-1 text-xs sm:text-sm">
                                            ...
                                            <p><span className="font-medium">{t('doctorPages.requestingDoctor') || 'Requesting Doctor'}:</span> {selectedImage.requesting_doctor || 'N/A'}</p>
                                    ...
                                    {selectedImage.notes && (
                                        <div>
                                            <h3 className="font-medium text-gray-900 mb-2 text-sm sm:text-base">
                                                {t('audiometry.notes.title') || 'Notes'}
                                            </h3>
...
export default DoctorAudiometryPage;
