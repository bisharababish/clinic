-- BULLETPROOF FIX - No more recursion
...
DROP POLICY IF EXISTS "auth_users_foreign_key_access" ON auth.users;

ALTER TABLE auth.users DISABLE ROW LEVEL SECURITY;
ALTER TABLE auth.users ENABLE ROW LEVEL SECURITY;

CREATE POLICY "auth_users_no_recursion" ON auth.users
    FOR ALL USING (true) WITH CHECK (true);

-- Step 6: Fix payment_bookings
...
-- Step 7: Fix payment_transactions
