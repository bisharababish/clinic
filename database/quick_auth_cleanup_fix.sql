-- QUICK AUTH CLEANUP FIX
-- This creates a simple function that deletes users and handles auth cleanup
...
-- =====================================================
-- 2. CREATE WORKING USER DELETION WITH AUTH CLEANUP
-- =====================================================
...
BEGIN
    ...
    BEGIN
        ...
        -- 17. Delete userinfo record (this should cascade to auth.users via foreign key constraint)
        -- The userinfo table has: CONSTRAINT userinfo_id_fkey FOREIGN KEY (id) REFERENCES auth.users(id)
        -- So deleting from userinfo should automatically delete from auth.users
        ...
        -- 18. Verify auth.users deletion (should be automatic via foreign key)
        BEGIN
            -- Check if auth user still exists after userinfo deletion
            SELECT COUNT(*) INTO result_count FROM auth.users WHERE id = auth_user_id;
            IF result_count = 0 THEN
                auth_deletion_success := true;
                deletion_results := deletion_results || json_build_object('auth_users', 1);
            ELSE
                -- Try manual deletion if foreign key didn't work
...
-- =====================================================
-- 3. CREATE AUTH CLEANUP FUNCTION
-- =====================================================
